"""
app_supabase.py
Same Return-Risk Verifier webpage as app.py, but reading order data from a
real Supabase (hosted Postgres) database instead of local CSV files - a step
toward how this would actually run in production, where data lives in a
database that multiple services can query, not a file on one laptop.

SETUP (one-time):
  1. Create a free Supabase project at supabase.com
  2. In Supabase: SQL Editor -> New Query -> paste schema.sql -> Run
  3. Run: python upload_to_supabase.py   (uploads test.csv, razorpay_ids.csv,
     cross_merchant_network.csv into the 3 tables - see that file for setup)
  4. pip install flask ollama pandas scikit-learn joblib supabase

USAGE:
  Set these two environment variables (PowerShell), same ones from step 3:
    $env:SUPABASE_URL="https://your-project.supabase.co"
    $env:SUPABASE_KEY="your-anon-public-key"
  Then:
    python app_supabase.py
  Open http://localhost:5000 in your browser - functionally identical to
  app.py from the outside, but every lookup is now a real database query.

NOTE: the ML model itself (risk_model.joblib) stays a local file, not a
database row - that's realistic too: models are usually served from a model
registry/artifact store, separate from the transactional database. Only the
order/payment/network DATA moved to Supabase, which is the part that's
actually relational, queryable data.
"""
import inspect
import json
import os
...
MODEL = joblib.load("risk_model.joblib")  # model stays local, see note above
...
if not SUPABASE_URL or not SUPABASE_KEY:
    raise SystemExit(
        ...
        "Set them first (see the top of this file for instructions)."
    )
...
def fetch_order(order_id: str):
    """One real database query per order lookup, instead of reading a whole
    CSV into memory. This is a small, single-row query - realistic scale."""
    res = supabase.table("orders").select("*").eq("order_id", order_id).execute()
    return res.data[0] if res.data else None


def fetch_razorpay_ids(order_id: str):
    res = supabase.table("razorpay_ids").select("*").eq("order_id", order_id).execute()
    return res.data[0] if res.data else None


def resolve_payment_id(payment_id: str):
    """A Razorpay analyst has a payment ID, not the internal order_id - this
    is a real indexed database lookup (see idx_payment_id in schema.sql),
    not a linear scan through a file."""
    res = supabase.table("razorpay_ids").select("order_id").eq("razorpay_payment_id", payment_id).execute()
    return res.data[0]["order_id"] if res.data else None
...
def check_cross_merchant_pattern(order_id: str) -> dict:
    match = supabase.table("cross_merchant_network").select("*").eq("order_id", order_id).execute()
    if not match.data:
        return {"result": "No network record for this order."}
    account = match.data[0]["refund_destination_account"]
    this_merchant = match.data[0]["merchant_id"]

    # THE key cross-merchant query: find every order sharing this same
    # refund destination, regardless of merchant - this is the query a
    # single merchant's own database could never run, since they'd only
    # ever see their own rows. A shared Supabase table can.
    same = supabase.table("cross_merchant_network").select("merchant_id").eq(
        "refund_destination_account", account
    ).execute()
    merchants = sorted(set(row["merchant_id"] for row in same.data))
...
def policy_decision(order_id: str) -> dict:
    """Fetch this order's evidence via Supabase-backed tools, then apply the
    shared policy from decision_policy.py (same function every entry point uses)."""
...
def resolve_order_id(search_id: str):
    ...
    reference to the internal order_id, via real database queries."""
    if search_id.startswith("pay_"):
        return resolve_payment_id(search_id)
    o = fetch_order(search_id)
    return search_id if o is not None else None
...
def review_order(order_id: str) -> dict:
    ...
    network = check_cross_merchant_pattern(order_id)
    o = fetch_order(order_id)
    actual_label = "ABUSE" if int(o["is_abuse"]) == 1 else "LEGITIMATE"
...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .mic-btn{background:var(--panel); color:var(--text); border:1px solid var(--line); padding:12px 16px; font-size:16px;}
  .mic-btn.listening{background:var(--amber); color:#1a1300; animation:pulse 1s infinite;}
  @keyframes pulse{0%,100%{opacity:1;}50%{opacity:0.6;}}
  .speak-btn{background:var(--panel); color:var(--teal); border:1px solid var(--line); font-size:12px; padding:6px 12px; margin-top:10px; font-family:var(--mono);}
...
<div class="wrap">
  ...
  <div class="searchbar">
    ...
    <button id="mic" class="mic-btn" title="Speak the payment ID">🎤</button>
    <button id="go">Review</button>
  </div>
  <p class="sub" id="voiceStatus" style="font-size:12px; min-height:16px;"></p>
...
// --- Voice input: speak a payment ID instead of typing it ---
...
if (SpeechRecognitionAPI) {
  ...
  micBtn.addEventListener('click', () => {
    ...
    voiceStatus.textContent = 'Listening... say the payment ID (e.g. "pay y nine dee zee em nine...")';
    recognition.start();
  });

  recognition.onresult = (event) => {
    const transcript = event.results[0][0].transcript.trim();
    // Voice-to-text rarely produces a clean "pay_xxxx" string, so we clean
    // it up: lowercase, strip spaces, and prefix with pay_ if it's missing -
    // this is a best-effort convenience, not a claim of perfect accuracy.
    let cleaned = transcript.toLowerCase().replace(/\s+/g, '');
    if (!cleaned.startsWith('pay_') && !cleaned.startsWith('pay')) {
      cleaned = 'pay_' + cleaned;
    } else if (cleaned.startsWith('pay') && !cleaned.startsWith('pay_')) {
      cleaned = 'pay_' + cleaned.slice(3);
    }
    input.value = cleaned;
    voiceStatus.textContent = 'Heard: "' + transcript + '" \u2192 typed as: ' + cleaned + ' (edit if needed, voice-to-ID matching is best-effort)';
  };
  recognition.onerror = (event) => {
    voiceStatus.textContent = 'Voice input error: ' + event.error + '. You can still type the ID directly.';
  };
...
// --- Voice output: read the decision aloud ---
function speakDecision(dec, actualLabel) {
  if (!window.speechSynthesis || !dec) return;
  const utterance = new SpeechSynthesisUtterance(
    `Decision: ${dec.action.replace(/_/g, ' ')}. Reasoning: ${dec.reasoning}`
  );
  utterance.rate = 1.0;
...
async function review(){
  ...
  try{
    ...
    card.innerHTML = `
      ...
      <button class="speak-btn" id="speakBtn">🔊 Read decision aloud</button>
    `;
    card.style.display='block';
    document.getElementById('speakBtn').addEventListener('click', () => speakDecision(dec, data.actual_label));
  }catch(e){
    loading.style.display='none';
    go.disabled = false;
